    ...
    <div className={styles.page}>
      ...
      <main className={styles.main}>
        ...
        <Section id="install" title="安装说明" icon={
          ...
          <SubSection title="macOS 安装">
            ...
            <Step num={1} title="安装应用程序">
              <p className={styles.text}>双击 <code>Lucencia.dmg</code> 安装包，将应用图标拖拽到"应用程序"文件夹即可完成安装。</p>
            </Step>

            <Step num={2} title="配置系统权限">
        ...
        <Section id="settings" title="重要设置" icon={
          ...
          <SubSection title="邀请奖励机制">
            <p className={styles.text}>邀请奖励采用延迟结算机制，不会在好友注册时立即发放。被邀请账号需要完成注册，并在桌面客户端首次登录或上报设备指纹后，系统确认设备与登录环境正常，才会给邀请人与被邀请人发放赠送套餐。</p>
            <ul className={styles.list}>
              <li><strong>正常结算：</strong>被邀请人通过邀请码注册，并使用自己的设备登录客户端后，双方获得待激活赠送套餐。</li>
              <li><strong>风控撤回：</strong>如果邀请人与被邀请人使用同一台设备，或邀请链路出现批量同设备、同 IP 登录，系统会撤回相关邀请赠送权益。</li>
              <li><strong>账号冻结：</strong>被判定为小号薅羊毛的被邀请账号会被冻结；同一邀请人多次触发异常时，邀请人账号也可能被冻结。</li>
            </ul>
            <Warn>请不要使用自己的设备批量注册小号或互相邀请。系统会在注册、登录和设备上报时持续检查邀请链路，异常奖励即使已发放也会被撤回。</Warn>
          </SubSection>
